...
public class FireAlarmControllerService {
    ...
    @Path("/switchbulb") @POST public void switchBulb(@HeaderParam("owner") String owner,
        ...
        try {
            boolean result = DeviceController.setControl(owner, "FireAlarm", deviceId, "BULB", "IN");
            if (result) {
    ...
    @Path("/readtemperature") @GET public String requestTemperature(@HeaderParam("owner") String owner,
        ...
        String replyMsg;
        try {
            boolean result = DeviceController.setControl(owner, "FireAlarm", deviceId, "TEMPERATURE", "IN");
            if (result) {
                response.setStatus(HttpStatus.SC_ACCEPTED);
    ...
    @Path("/togglefan") @POST public void switchFan(@HeaderParam("owner") String owner,
        ...
        try {
            boolean result = DeviceController.setControl(owner, "FireAlarm", deviceId, "FAN", "IN");
            if (result) {
                response.setStatus(HttpStatus.SC_ACCEPTED);
        ...
        try {
            if (sensors.length == 3) {
                ...
                result = DeviceController
                        .pushData(dataMsg.owner, "FireAlarm", dataMsg.deviceId, System.currentTimeMillis(),
